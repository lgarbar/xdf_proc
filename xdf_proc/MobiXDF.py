...
def preproc_physio_df(phys: pd.DataFrame) -> pd.DataFrame:
    ...
    proc_map = {}
    proc_map["ecg"] = {"col": "ECG0", "proc": bp.signals.ecg.ecg}
    proc_map["eda"] = {"col": "EDA1", "proc": bp.signals.eda.eda}
    proc_map["resp"] = {"col": "RESPIRATION4", "proc": bp.signals.resp.resp}
    proc_map["abs_motion"] = {
        "col": ["XYZ5", "XYZ6", "XYZ7"],
        "proc": abs_motion_change,
    }

    proc_map["emg1"] = {"col": "EMG2", "proc": bp.signals.emg.emg}
    proc_map["emg2"] = {"col": "EMG3", "proc": bp.signals.emg.emg}
