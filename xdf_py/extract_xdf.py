...
def process_xdf_file_and_update_log(xdf_file: str, dest_folder: str, modalities: list) -> None:
    ...
    if all(x == 1 for x in modalities):
        res = process_xdf_file(xdf_file, dest_folder)
    else:
        process_xdf_modalities(modalities, xdf_file, dest_folder)
    
    update_process_log(xdf_file, isinstance(res, pd.DataFrame))

def process_xdf_list(xdf_list: list, dest_folder:str, modalities:list, max_workers:int=4):
    """
    ...
    with ProcessPoolExecutor(max_workers) as executor:
        executor.map(process_xdf_file_and_update_log, xdf_list, [dest_folder]*len(xdf_list), [modalities]*len(xdf_list))
...
def main():
    ...
    process_xdf_list(xdf_list, args.dest_folder, args.modalities, args.max_workers)
